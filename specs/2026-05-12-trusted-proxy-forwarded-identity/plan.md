...
4. In `src/auth.py` add `async def _trusted_proxy_response(request, call_next) -> Response | None`. Returns `None` immediately when either `JENTIC_TRUSTED_PROXY_HEADER` or `JENTIC_TRUSTED_PROXY_NETS` is empty (feature inactive). Reads peer IP from `request.client.host` (never `X-Forwarded-For`). On untrusted peer + header present, log `logger.warning("PROXY_AUTH untrusted_peer=%s header=%s ignored", peer_ip, JENTIC_TRUSTED_PROXY_HEADER)` — log the configured header **name** (not the value it carried) to avoid persisting identity tokens in logs — and return `None` so the next auth path runs. On trusted peer + header present, read `proxy_identity = request.headers.get(JENTIC_TRUSTED_PROXY_HEADER, "").strip()`, then JIT-provision the user via `INSERT OR IGNORE INTO users (id, username, password_hash, created_via) VALUES (?, ?, NULL, 'trusted_proxy')` where `id = str(uuid.uuid4())` (import `uuid` from stdlib); `SELECT` to fetch the canonical row, set `request.state.is_human_session = True`, `request.state.is_admin = True`, `request.state.toolkit_id = DEFAULT_TOOLKIT_ID`, `request.state.username = proxy_identity` (so `/user/me` returns the correct identity rather than the first DB row), and return `await call_next(request)`. On trusted peer + no header, return `None`.
...
5a. In `src/routers/user.py`, update the `GET /user/me` handler's `is_human_session` branch (`src/routers/user.py:342–346`): return `request.state.username` when present on the request state instead of querying `SELECT username FROM users LIMIT 1`. This is required because multiple JIT-provisioned rows can exist once Phase 28 ships and `LIMIT 1` would return an arbitrary row. Pattern: `username = getattr(request.state, "username", None) or (await db query LIMIT 1)[0]` — the DB fallback preserves the existing behaviour for JWT-authenticated sessions where `request.state.username` is not set.
...
9. In `src/routers/user.py` login handler (~line 192), replace the existing `if not row or not bcrypt.checkpw(...)` check with `if not row or row["password_hash"] is None or not bcrypt.checkpw(password.encode(), row["password_hash"].encode())`. On failure, raise `HTTPException(401, detail={"error": "invalid_credentials", "message": "Username or password incorrect."})` — same shape as the current failed-login response at line 194 (FastAPI serializes `detail=dict` as `{"detail": {...}}`), same shape regardless of which sub-condition fired (no enumeration leak).

10. Apply the identical `password_hash is None` guard in the `/user/token` OAuth2 password-grant handler (~line 276), using `HTTPException(401, detail={"error": "invalid_client", "error_description": "Invalid username or password."})` — matching the existing OAuth error shape at that handler, keeping the two endpoints' error formats consistent with their respective current conventions.
...
12. Create `tests/test_trusted_proxy_auth.py` covering the six #366 acceptance criteria plus prefix-gating: (a) no-env passthrough; (b) trusted peer + header → 200 + identity + JIT row created with `password_hash IS NULL` and `created_via='trusted_proxy'`; (c) trusted peer + no header → 401; (d) untrusted peer + header → 401 + WARN log on `jentic.auth`; (e) trusted peer + spoofed `X-Forwarded-For` outside CIDR → still authenticated (peer IP is the gate); (f) JIT-provisioned account at `/user/login` → 401 with `{"detail": {"error": "invalid_credentials", ...}}`; at `/user/token` → 401 with `{"detail": {"error": "invalid_client", ...}}`; no 500 on either; (g) trusted peer + `X-Forwarded-Prefix: /foo` → `root_path` applied; (h) untrusted peer + same prefix header → `root_path` stays unset + WARN log. Use `TestClient(app, client=(ip, port))` to fake peer addresses; `monkeypatch.setenv` to toggle env vars.

13. Update `docs/auth.md`: add a "Trusted-Proxy Forwarded Identity" subsection under Human Authentication describing the env vars, the CIDR-gate security boundary, JIT provisioning, and the NULL-hash login rejection. Update the `users` schema documentation in "Database tables" to show `created_via` and NULLable `password_hash`. Remove or revise the "Single root account — no multi-user, no roles. One human owns the instance." paragraph; Phase 28 introduces JIT-provisioned rows per SSO identity, making that claim inaccurate.
