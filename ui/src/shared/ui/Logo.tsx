 ...
 * `@jentic/frontend-ui`'s `SvgLogo` so jentic-one is pixel-identical to the
 * webapp. The "One" badge is added to distinguish this product variant.
 */
...
export interface JenticLogoProps {
	...
	showWord?: boolean;
	/** Show the "One" product badge. Defaults to true. */
	showBadge?: boolean;
	className?: string;
}
...
export function JenticLogo({
	...
	return (
		<div className={`flex items-center gap-2${className ? ` ${className}` : ''}`}>
			...
			{showBadge && (
				// `aria-hidden`: the closest interactive ancestor (the
				// `TopNavbar` home link) carries its own accessible name like
				// "Jentic One home", so re-announcing "One" here would just be
