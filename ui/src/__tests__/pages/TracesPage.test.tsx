...
describe('TracesPage', () => {
	...
	it('shows empty state when no traces exist', async () => {
		worker.use(http.get('/traces', () => HttpResponse.json({ traces: [], total: 0 })));

		renderWithProviders(<TracesPage />);

		expect(await screen.findByText('No traces found')).toBeInTheDocument();
		expect(
			screen.getByText(/Traces appear here when agents call the broker/),
		).toBeInTheDocument();
	});
	...
	it('shows error state when API returns 500', async () => {
		worker.use(createErrorHandler('get', '/traces', { status: 500 }));
