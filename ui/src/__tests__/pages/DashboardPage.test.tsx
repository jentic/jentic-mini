...
describe('DashboardPage', () => {
  ...
  it('shows stat values with populated data', async () => {
    worker.use(
      http.get('/apis', () =>
        HttpResponse.json({ data: [{ id: 'a1' }], total: 5, page: 1 }),
      ),
      http.get('/toolkits', () =>
        HttpResponse.json([{ id: 'tk1', name: 'Stripe' }, { id: 'tk2', name: 'GitHub' }]),
      ),
      http.get('/workflows', () =>
        HttpResponse.json([{ slug: 'w1' }]),
      ),
      http.get('/traces', () =>
        HttpResponse.json({ traces: [], total: 42 }),
      ),
    )
  ...
  it('handles API error gracefully (does not crash)', async () => {
    worker.use(
      http.get('/apis', () => HttpResponse.error()),
    )
