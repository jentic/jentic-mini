...
describe('WorkflowsPage', () => {
	...
	it('renders workflow list with populated data', async () => {
		worker.use(
			http.get('/workflows', () =>
				HttpResponse.json([
					{
						slug: 'summarise-discourse',
						name: 'Summarise Discourse Topics',
						description: 'Summarises recent topics',
						source: 'local',
						steps_count: 3,
						involved_apis: ['discourse-api'],
					},
					{
						slug: 'sync-contacts',
						name: 'Sync Contacts',
						...
						source: 'catalog',
						steps_count: 5,
						involved_apis: ['salesforce', 'hubspot'],
					},
				]),
			),
		);
	...
	it('shows error state when API fails', async () => {
		worker.use(createErrorHandler('get', '/workflows', { status: 500 }));
