...
describe('OAuthBrokersPage', () => {
  ...
  it('renders broker cards with populated data', async () => {
    worker.use(
      http.get('/oauth-brokers', () =>
        HttpResponse.json([
          { id: 'pipedream-1', type: 'pipedream', config: { project_id: 'proj-abc', default_external_user_id: 'default' }, created_at: String(Math.floor(Date.now() / 1000)) },
          { id: 'pipedream-2', type: 'pipedream', config: { project_id: 'proj-xyz', default_external_user_id: 'agent-1' }, created_at: String(Math.floor(Date.now() / 1000)) },
        ]),
      ),
    )
  ...
  it('shows error on failed broker creation', async () => {
    worker.use(
      createErrorHandler('post', '/oauth-brokers', { status: 500, body: { detail: 'Server error' } }),
    )
