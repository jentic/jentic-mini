    ...
    **`type=modify_permissions`** — update rules on a credential already bound to this toolkit:
    ...
    rules: list[PermissionRule] = Field(
        ...
        description=(
            ...
            "- `path`: Python regex, `re.search()` substring match, case-insensitive. `|` is OR. "
            "E.g. `\"text-to-speech\"` matches any path containing that string; `\"admin|billing\"` blocks both.\n"
