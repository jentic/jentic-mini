...
Tests check_policy directly (pure function) and verifies the
system safety rules behave as documented.
"""

from src.routers.toolkits import check_policy
...
class TestSystemSafetyRulesAlone:
    ...
    def test_denies_post(self):
        allowed, _ = check_policy([], None, method="POST", path="/v1/pages")
        assert not allowed

    def test_denies_put(self):
        allowed, _ = check_policy([], None, method="PUT", path="/v1/pages/123")
        assert not allowed

    def test_denies_patch(self):
        allowed, _ = check_policy([], None, method="PATCH", path="/v1/pages/123")
        assert not allowed

    def test_denies_delete(self):
        allowed, _ = check_policy([], None, method="DELETE", path="/v1/pages/123")
        assert not allowed

    def test_allows_get(self):
        allowed, _ = check_policy([], None, method="GET", path="/v1/pages")
        assert allowed
    ...
    def test_denies_sensitive_paths_even_for_get(self):
        for path in [
            ...
            allowed, _ = check_policy([], None, method="GET", path=path)
...
class TestAgentRulesOverride:
    ...
    def test_allow_rule_grants_write(self):
        ...
        allowed, _ = check_policy(rules, None, method="POST", path="/v1/pages")
        assert allowed

    def test_allow_rule_does_not_match_other_paths(self):
        rules = [{"effect": "allow", "methods": ["POST"], "path": "pages"}]
        allowed, _ = check_policy(rules, None, method="POST", path="/v1/users")
        assert not allowed  # falls through to system deny

    def test_allow_rule_does_not_match_other_methods(self):
        rules = [{"effect": "allow", "methods": ["POST"], "path": "pages"}]
        allowed, _ = check_policy(rules, None, method="DELETE", path="/v1/pages/123")
...
class TestPathRegexMatching:
    ...
    def test_substring_match(self):
        ...
        allowed, _ = check_policy(rules, None, method="POST", path="/gmail/v1/users/me/drafts")
        assert allowed

    def test_no_match_on_unrelated_path(self):
        rules = [{"effect": "allow", "methods": ["POST"], "path": "drafts"}]
        allowed, _ = check_policy(
